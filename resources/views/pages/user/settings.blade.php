<x-layouts.main title="Pengaturan">
    <div class="card">
        <div class="flex flex-wrap">
            <div class="border-e border-gray-200">
                <nav class="flex flex-col space-y-2" aria-label="Tabs" role="tablist" aria-orientation="horizontal">
                    <button type="button"
                        class="hs-tab-active:border-blue-500 hs-tab-active:text-blue-600 py-1 pe-4 inline-flex items-center gap-x-2 border-e-2 border-transparent text-sm whitespace-nowrap text-gray-500 hover:text-blue-600 focus:outline-hidden focus:text-blue-600 disabled:opacity-50 disabled:pointer-events-none active"
                    ...
                    <button type="button"
                        class="hs-tab-active:border-blue-500 hs-tab-active:text-blue-600 py-1 pe-4 inline-flex items-center gap-x-2 border-e-2 border-transparent text-sm whitespace-nowrap text-gray-500 hover:text-blue-600 focus:outline-hidden focus:text-blue-600 disabled:opacity-50 disabled:pointer-events-none"
            ...
            <div class="ms-3 grow">
                <div id="vertical-tab-with-border-1" role="tabpanel" aria-labelledby="vertical-tab-with-border-item-1">
                    <div class="lg:grid grid-cols-5">
                        <form action="{{ route('settings.account') }}" method="post" class="mt-5 col-span-2"
                            autocomplete="off">
                            @method('PUT')
                            @csrf
                                ...
                                <select id="divisionSelect" name="division_id"
                                    ...
                                    @foreach ($divisions as $division)
                                        <option value="{{ $division->id }}"
                                            @if ($division->id == $user->division_id) selected @endif>{{ $division->nama }}
                                        </option>
                                    @endforeach
                                </select>
                            </div>
                        ...
                        <div class="text-center hidden lg:block col-span-3">
                            <img src="{{ asset('assets/svg/oc-project-development.svg') }}" alt=""
                ...
                <div id="vertical-tab-with-border-2" class="hidden" role="tabpanel"
                    ...
                    <div class="lg:grid grid-cols-5">
                        <form action="{{ route('settings.password') }}" method="post" class="mt-5 col-span-2"
                            autocomplete="off">
                            @method('PUT')
                            @csrf
